...
function calculateVWAP(orders: OrderData[]): { rate: number; isFallback: boolean } {
...
async function processCorridors(): Promise<void> {
  ...
  for (const corridorId of corridors) {
    try {
      ...
      // 5-minute volume
      const volResult = await dbQuery<{ v: string }>(
        `SELECT COALESCE(SUM(crypto_amount), 0)::text as v
         FROM orders
         WHERE status = 'completed'
      ...
      await dbQuery(
        `UPDATE corridor_prices
         SET ref_price = $2,
             ...
             updated_at = NOW()
         WHERE corridor_id = $1`,
        [corridorId, refPrice, volume5m, confidence]
      );
...
function writeHeartbeat(corridorId: string, price: number, confidence: string, ordersAnalyzed: number): void {
  try {
...
export function startPriceFeedWorker(): void {
  if (isRunning) return;
  isRunning = true;
...
export function stopPriceFeedWorker(): void {
  isRunning = false;
  if (pollTimer) {
