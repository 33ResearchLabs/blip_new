 ...
 * Core API Auth Hook
 *
 * Validates x-core-api-secret header on all requests except /health.
 * When actor identity headers are present, verifies HMAC signature
 * to prevent actor forgery.
 * If CORE_API_SECRET is not set, auth is disabled (local dev compatibility).
 */
import type { FastifyPluginAsync } from 'fastify';
import { createHmac, timingSafeEqual } from 'crypto';

export const authHook: FastifyPluginAsync = async (fastify) => {
