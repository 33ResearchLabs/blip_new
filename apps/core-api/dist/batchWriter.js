...
async function flush() {
    ...
    if (events.length > 0) {
        ...
        query(`INSERT INTO order_events (order_id,event_type,actor_type,actor_id,old_status,new_status,metadata) VALUES ${phs.join(',')}`, vals).catch(() => { });
    }
    // Flush notifications
    if (notifs.length > 0) {
        const vals = [];
        ...
        query(`INSERT INTO notification_outbox (order_id,event_type,payload,status) VALUES ${phs.join(',')}`, vals).catch(() => { });
    }
    // Flush reputation
    if (reps.length > 0) {
        const vals = [];
        ...
        query(`INSERT INTO reputation_events (entity_id,entity_type,event_type,score_change,reason,metadata) VALUES ${phs.join(',')} ON CONFLICT DO NOTHING`, vals).catch(() => { });
    }
}
export function bufferEvent(row) {
    eventBuf.push(row);
    if (eventBuf.length >= MAX_BUF)
        flush();
    else
        schedule();
}
export function bufferNotification(row) {
    notifBuf.push(row);
    if (notifBuf.length >= MAX_BUF)
        flush();
    else
        schedule();
}
export function bufferReputation(row) {
    repBuf.push(row);
    if (repBuf.length >= MAX_BUF)
        flush();
    else
        schedule();
}
