import { query as dbQuery, queryOne, transaction, atomicCancelWithRefund, verifyReleaseInvariants, verifyRefundInvariants, validateTransition, normalizeStatus, isTransientStatus, getTransitionEventType, shouldRestoreLiquidity, logger, MOCK_MODE, } from 'settlement-core';
// Fire-and-forget helper — logs errors but never blocks
const bgQuery = (sql, params) => dbQuery(sql, params).catch(() => { });
    ...
    fastify.patch('/orders/:id', async (request, reply) => {
        ...
        try {
            ...
            if (newStatus !== 'cancelled' && newStatus !== 'accepted') {
                const selfRefCheck = await queryOne(`SELECT id FROM orders WHERE id = $1 AND merchant_id = buyer_merchant_id`, [id]);
                if (selfRefCheck) {
            ...
            if (newStatus === 'cancelled') {
                ...
                if (order.escrow_tx_hash) {
                    const cancelResult = await atomicCancelWithRefund(id, order.status, actor_type, actor_id, reason, {
                        type: order.type,
            ...
            if (newStatus === 'accepted' || (newStatus === 'payment_pending' && request.body.metadata?.is_m2m)) {
                ...
                if (actor_type === 'merchant') {
                    const preCheck = await queryOne('SELECT o.merchant_id, o.type, o.escrow_tx_hash, o.crypto_amount, o.buyer_merchant_id, u.username FROM orders o JOIN users u ON u.id = o.user_id WHERE o.id = $1', [id]);
                    ...
                    if (acceptorIsSeller) {
                        const merchantBalance = await queryOne('SELECT balance FROM merchants WHERE id = $1', [actor_id]);
                ...
                const procResult = await queryOne('SELECT accept_order_v1($1,$2,$3,$4)', [id, actor_type, actor_id, request.body.acceptor_wallet_address || null]);
                ...
                broadcastOrderEvent({
                    ...
                    minimal_status: normalizeStatus(order.status), order_version: order.order_version,
            ...
            if (newStatus === 'payment_sent') {
                const updated = await queryOne(`UPDATE orders SET status = 'payment_sent', payment_sent_at = NOW(), order_version = order_version + 1
           ...
           RETURNING *`, [id]);
            ...
            const result = await transaction(async (client) => {
                ...
                const shouldRelist = newStatus === 'cancelled' &&
                    actor_type === 'merchant' &&
                    oldStatus === 'accepted' &&
                    !currentOrder.escrow_tx_hash;
                if (shouldRelist) {
                    const relistValidation = validateTransition(oldStatus, 'pending', actor_type);
                    if (!relistValidation.valid) {
                        return { success: false, error: relistValidation.error };
                    }
             ...
             VALUES ($1, 'merchant_relisted', $2, $3, 'accepted', 'pending', $4)`, [id, actor_type, actor_id, JSON.stringify({ reason: reason || 'Merchant cancelled — relisted to marketplace' })]);
                    await client.query(`INSERT INTO notification_outbox (order_id, event_type, payload, status) VALUES ($1, 'ORDER_RELISTED', $2, 'pending')`, [
                        id,
                        JSON.stringify({
                            orderId: id,
                    ...
                    if (shouldRestoreLiquidity('accepted', 'pending')) {
                    ...
                    return { success: true, order: updatedOrder, oldStatus, currentOrder, relisted: true };
                }
                // Validate transition
                ...
                if (actor_type === 'merchant' &&
                    ...
                    const userResult = await client.query('SELECT username FROM users WHERE id = $1', [currentOrder.user_id]);
                ...
                if (newStatus === 'completed' && currentOrder.payment_via === 'saed_corridor' && currentOrder.corridor_fulfillment_id) {
                    try {
                        const ffResult = await client.query('SELECT * FROM corridor_fulfillments WHERE id = $1 FOR UPDATE', [currentOrder.corridor_fulfillment_id]);
                        if (ffResult.rows.length > 0 && ffResult.rows[0].provider_status !== 'completed') {
                            ...
                            await client.query('UPDATE merchants SET sinr_balance = sinr_balance + $1 WHERE id = $2', [saedAmount, providerMerchantId]);
                            await client.query(`UPDATE corridor_fulfillments SET provider_status = 'completed', completed_at = NOW(), updated_at = NOW() WHERE id = $1`, [currentOrder.corridor_fulfillment_id]);
                        }
                    }
                    catch (corridorErr) {
            ...
            if (newStatus === 'completed') {
                bgQuery(`WITH u AS (UPDATE users SET total_trades = total_trades + 1, total_volume = total_volume + $1 WHERE id = $2 RETURNING 1)
           UPDATE merchants SET total_trades = total_trades + 1, total_volume = total_volume + $1 WHERE id = $3`, [txOrder.fiat_amount, txOrder.user_id, txOrder.merchant_id]);
            }
    ...
    fastify.delete('/orders/:id', async (request, reply) => {
        ...
        try {
            ...
            if (order.escrow_tx_hash) {
                ...
                const shouldRelist = actor_type === 'merchant' &&
                ...
                if (shouldRelist) {
                    const result = await transaction(async (client) => {
                        ...
                        if (currentOrder.status !== 'accepted' || currentOrder.escrow_tx_hash) {
                            return { success: false, error: 'Order state changed, cannot relist' };
                        }
                        const validation = validateTransition(currentOrder.status, 'pending', actor_type);
                        if (!validation.valid) {
                            return { success: false, error: validation.error };
                        }
                        const updateResult = await client.query(`UPDATE orders
    ...
    fastify.post('/orders/:id/events', async (request, reply) => {
        ...
        try {
            if (event_type === 'release') {
                ...
                const procResult = await queryOne('SELECT release_order_v1($1,$2,$3)', [id, tx_hash, MOCK_MODE]);
                ...
                const refundResult = await atomicCancelWithRefund(id, order.status, actorType, actorId, reason, {
                    type: order.type,
                    crypto_amount: parseFloat(String(order.crypto_amount)),
                    merchant_id: order.merchant_id,
                    user_id: order.user_id,
