import { query as dbQuery, queryOne, normalizeStatus, logger, } from 'settlement-core';
import { broadcastOrderEvent } from '../ws/broadcast';
export const disputeRoutes = async (fastify) => {
    // POST /v1/orders/:id/dispute - Open dispute
    fastify.post('/orders/:id/dispute', async (request, reply) => {
        ...
        try {
            // Check order exists
            const order = await queryOne('SELECT id, status, user_id, merchant_id FROM orders WHERE id = $1', [id]);
            if (!order) {
        ...
        `);
            ...
            // Insert dispute
            const disputeResult = await dbQuery(`INSERT INTO disputes (
         ...
         RETURNING *`, [id, reason, description || '', initiated_by, actor_id]);
            // Update order status
            await dbQuery(`UPDATE orders SET status = 'disputed'::order_status, order_version = order_version + 1 WHERE id = $1`, [id]);
            // Event
            await dbQuery(`INSERT INTO order_events (order_id, event_type, actor_type, actor_id, old_status, new_status, metadata)
         VALUES ($1, 'status_changed_to_disputed', $2, $3, $4, 'disputed', $5)`, [id, initiated_by, actor_id, order.status, JSON.stringify({ reason, description })]);
            // Notification outbox
            await dbQuery(`INSERT INTO notification_outbox (order_id, event_type, payload, status) VALUES ($1, 'ORDER_DISPUTED', $2, 'pending')`, [
                id,
                JSON.stringify({
                    orderId: id,
            ...
            broadcastOrderEvent({
                ...
                order_id: id,
                status: 'disputed',
                minimal_status: normalizeStatus('disputed'),
    ...
    fastify.post('/orders/:id/dispute/confirm', async (request, reply) => {
        const { id } = request.params;
        ...
        try {
            const disputeResult = await dbQuery(`SELECT d.*, o.user_id, o.merchant_id
         ...
         WHERE d.order_id = $1`, [id]);
           ...
           WHERE order_id = $1`, [id]);
            ...
            const updated = await queryOne('SELECT user_confirmed, merchant_confirmed, proposed_resolution FROM disputes WHERE order_id = $1', [id]);
            if (updated && updated.user_confirmed && updated.merchant_confirmed) {
                ...
                const orderResult = await dbQuery(`SELECT o.*, d.split_percentage FROM orders o JOIN disputes d ON d.order_id = o.id WHERE o.id = $1`, [id]);
                ...
                await dbQuery(`UPDATE disputes SET status = 'resolved'::dispute_status, resolution = $1, resolved_at = NOW() WHERE order_id = $2`, [resolution, id]);
                await dbQuery(`UPDATE orders SET status = $1::order_status, order_version = order_version + 1 WHERE id = $2`, [orderStatus, id]);
                // Notification outbox
                ...
                broadcastOrderEvent({
                    ...
                    order_id: id,
                    status: orderStatus,
                    minimal_status: normalizeStatus(orderStatus),
                    order_version: 0,
                    userId: order.user_id,
                    merchantId: order.merchant_id,
                    previousStatus: 'disputed',
