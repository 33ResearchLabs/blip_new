...
async function logLedgerEntry(client, accountType, accountId, direction, usdtAmount, usdtBalanceBefore, usdtBalanceAfter) {
    ...
    await client.query(`INSERT INTO ledger_entries
     ...
     VALUES ($1, $2, 'SYNTHETIC_CONVERSION', $3, 'USDT', $4, $5, $6)`, [accountType, accountId, ledgerAmount, description, usdtBalanceBefore, usdtBalanceAfter]);
}
/**
...
async function logMerchantTransaction(client, accountType, accountId, direction, usdtAmount, amountIn, amountOut, usdtBalanceBefore) {
    ...
    await client.query(`INSERT INTO merchant_transactions
     ...
     VALUES ($1, $2, 'manual_adjustment', $3, $4, $5, $6)`, [merchantId, userId, transactionAmount, usdtBalanceBefore, balanceAfter, description]);
}
export const conversionRoutes = async (fastify) => {
    // POST /v1/convert/usdt-to-sinr
    fastify.post('/convert/usdt-to-sinr', async (request, reply) => {
        ...
        try {
            const result = await transaction(async (client) => {
                ...
                // 1. Check idempotency
                if (idempotency_key) {
                    const existing = await client.query(`SELECT id, amount_in, amount_out, rate, usdt_balance_after, sinr_balance_after
             ...
             WHERE idempotency_key = $1`, [idempotency_key]);
                ...
                const lockResult = await client.query(`SELECT balance, sinr_balance, synthetic_rate, max_sinr_exposure
           FROM ${table}
           WHERE id = $1
           ...
           WHERE id = $3`, [newUsdtBalance, newSinrBalance, account_id]);
                // 5. Insert conversion record
                const convResult = await client.query(`INSERT INTO synthetic_conversions
           ...
           RETURNING id`, [account_type, account_id, amount, amountOut, rate,
                    usdtBalance, newUsdtBalance, sinrBalance, newSinrBalance, idempotency_key || null]);
                const conversionId = String(convResult.rows[0].id);
                // 6. Log ledger entry
                await logLedgerEntry(client, account_type, account_id, 'usdt_to_sinr', usdtAmount, usdtBalance, newUsdtBalance);
                // 7. Log merchant transaction
                await logMerchantTransaction(client, account_type, account_id, 'usdt_to_sinr', usdtAmount, amount, amountOut, usdtBalance);
    ...
    fastify.post('/convert/sinr-to-usdt', async (request, reply) => {
        const { account_type, account_id, amount, idempotency_key } = request.body;
        if (!account_type || !account_id || !amount) {
            return reply.status(400).send({
                success: false,
        ...
        try {
            const result = await transaction(async (client) => {
             ...
             WHERE idempotency_key = $1`, [idempotency_key]);
                ...
                // 2. Lock account and read state
                const lockResult = await client.query(`SELECT balance, sinr_balance, synthetic_rate
           FROM ${table}
           WHERE id = $1
           FOR UPDATE`, [account_id]);
                if (lockResult.rows.length === 0) {
           ...
           RETURNING id`, [account_type, account_id, amount, amountOut, rate,
                ...
                const conversionId = String(convResult.rows[0].id);
                // 6. Log ledger entry
                await logLedgerEntry(client, account_type, account_id, 'sinr_to_usdt', usdtAmount, usdtBalance, newUsdtBalance);
                // 7. Log merchant transaction
                await logMerchantTransaction(client, account_type, account_id, 'sinr_to_usdt', usdtAmount, amount, amountOut, usdtBalance);
