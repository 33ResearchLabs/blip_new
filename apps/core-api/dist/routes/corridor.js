...
export const corridorRoutes = async (fastify) => {
    // POST /v1/corridor/match — Find LP, lock buyer sAED, create fulfillment, link to order
    fastify.post('/corridor/match', async (request, reply) => {
        ...
        try {
            const result = await transaction(async (client) => {
                ...
                const lpResult = await client.query(`SELECT cp.*, m.rating as merchant_rating, m.display_name as merchant_name
           ...
           LIMIT 1
           FOR UPDATE`, [fiat_amount, buyer_merchant_id, seller_merchant_id]);
                ...
                // 2. Lock buyer's sAED
                const buyerResult = await client.query('SELECT sinr_balance FROM merchants WHERE id = $1 FOR UPDATE', [buyer_merchant_id]);
                if (buyerResult.rows.length === 0)
                    throw new Error('BUYER_NOT_FOUND');
                const buyerSaed = parseInt(String(buyerResult.rows[0].sinr_balance));
                if (buyerSaed < totalSaedLock)
                    throw new Error('INSUFFICIENT_SAED');
                await client.query('UPDATE merchants SET sinr_balance = sinr_balance - $1 WHERE id = $2', [totalSaedLock, buyer_merchant_id]);
           ...
           VALUES ('merchant', $1, 'CORRIDOR_SAED_LOCK', $2, 'sAED', $3, $4, $5, $6, $7)`, [
                ...
                await client.query(`INSERT INTO merchant_transactions
           (merchant_id, order_id, type, amount, balance_before, balance_after, description)
           VALUES ($1, $2, 'synthetic_conversion', $3, $4, $5, $6)`, [buyer_merchant_id, order_id, -totalSaedLock, buyerSaed, saedAfter, 'Corridor sAED lock']);
                // 5. Create fulfillment record
                const ffResult = await client.query(`INSERT INTO corridor_fulfillments
           ...
           WHERE id = $2`, [fulfillment.id, order_id]);
    ...
    fastify.patch('/corridor/fulfillments/:id', async (request, reply) => {
        ...
        try {
            const result = await transaction(async (client) => {
                const ffResult = await client.query('SELECT * FROM corridor_fulfillments WHERE id = $1 FOR UPDATE', [id]);
                if (ffResult.rows.length === 0)
                    throw new Error('NOT_FOUND');
                const ff = ffResult.rows[0];
                if (ff.provider_merchant_id !== actor_id)
                    throw new Error('UNAUTHORIZED');
                if (ff.provider_status !== 'pending')
                    throw new Error('INVALID_STATUS');
                await client.query(`UPDATE corridor_fulfillments
           ...
           RETURNING *`, [id]);
                ...
                await client.query(`INSERT INTO notification_outbox
           (order_id, event_type, merchant_id, payload)
           VALUES ($1, 'CORRIDOR_PAYMENT_SENT', $2, $3)`, [ff.order_id, ff.provider_merchant_id, JSON.stringify({ fulfillment_id: id })]);
            ...
            const msg = error.message;
            if (msg === 'NOT_FOUND')
                return reply.status(404).send({ success: false, error: 'Fulfillment not found' });
            if (msg === 'UNAUTHORIZED')
                return reply.status(403).send({ success: false, error: 'Not your fulfillment' });
            if (msg === 'INVALID_STATUS')
                return reply.status(400).send({ success: false, error: 'Fulfillment not in pending status' });
            fastify.log.error({ error, id }, 'Corridor fulfillment update failed');
            return reply.status(500).send({ success: false, error: 'Internal server error' });
        }
    });
    // GET /v1/corridor/fulfillments — LP's active assignments
    fastify.get('/corridor/fulfillments', async (request, reply) => {
        ...
        try {
            const rows = await transaction(async (client) => {
                const result = await client.query(`SELECT cf.*, o.order_number, o.crypto_amount, o.fiat_currency,
           ...
           ORDER BY cf.assigned_at DESC`, [provider_merchant_id]);
                return result.rows;
            });
    ...
    fastify.get('/corridor/providers', async (request, reply) => {
        ...
        try {
            const rows = await transaction(async (client) => {
                const result = await client.query('SELECT * FROM corridor_providers WHERE merchant_id = $1', [merchant_id]);
                return result.rows;
            });
    ...
    fastify.post('/corridor/providers', async (request, reply) => {
        ...
        try {
            const result = await transaction(async (client) => {
                const row = await client.query(`INSERT INTO corridor_providers (merchant_id, is_active, fee_percentage, min_amount, max_amount, auto_accept)
           VALUES ($1, $2, $3, $4, $5, $6)
           ON CONFLICT (merchant_id) DO UPDATE SET
             is_active = $2, fee_percentage = $3, min_amount = $4, max_amount = $5,
             auto_accept = $6, updated_at = NOW()
           RETURNING *`, [merchant_id, is_active, fee_percentage, min_amount, max_amount, auto_accept ?? true]);
                return row.rows[0];
            });
            return reply.status(200).send({ success: true, data: result });
        }
    ...
    fastify.get('/corridor/availability', async (request, reply) => {
        ...
        try {
            const result = await transaction(async (client) => {
                const row = await client.query(`SELECT COUNT(*) as cnt, MIN(cp.fee_percentage) as min_fee
           FROM corridor_providers cp
           JOIN merchants m ON cp.merchant_id = m.id
           WHERE cp.is_active = true
             AND m.is_online = true
             ...
             AND (cp.available_hours_start IS NULL
                  OR CURRENT_TIME BETWEEN cp.available_hours_start AND cp.available_hours_end)`, [fiatAmount, excludeIds.length > 0 ? excludeIds : null]);
