...
async function processOverdueFulfillments() {
    try {
        const count = await transaction(async (client) => {
            ...
            const result = await client.query(`SELECT cf.*, o.buyer_merchant_id
         ...
         LIMIT 10`);
           ...
           WHERE id = $1`, [fulfillmentId]);
                ...
                if (buyerMerchantId && saedAmount > 0) {
                    await client.query('UPDATE merchants SET sinr_balance = sinr_balance + $1 WHERE id = $2', [saedAmount, buyerMerchantId]);
           ...
           WHERE id = $1 AND corridor_fulfillment_id = $2`, [orderId, fulfillmentId]);
