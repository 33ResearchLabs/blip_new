...
async function sendTelegramNotification(merchantId, eventType, payload) {
    ...
    try {
        ...
        const merchantResult = await query(`SELECT telegram_chat_id FROM merchants WHERE id = $1`, [merchantId]);
...
async function processOutboxRecord(record) {
    try {
        // IDEMPOTENCY CHECK
        const statusCheck = await query('SELECT status FROM notification_outbox WHERE id = $1', [record.id]);
        ...
        // Send Telegram notification to merchant
        const telegramSuccess = await sendTelegramNotification(payload.merchantId, record.event_type, payload);
        if (!telegramSuccess) {
...
async function processBatch() {
    try {
        const records = await query(`SELECT * FROM notification_outbox
       ...
       FOR UPDATE SKIP LOCKED`, [BATCH_SIZE]);
        ...
        for (const record of records) {
            await query(`UPDATE notification_outbox
         ...
         WHERE id = $1`, [record.id]);
           ...
           WHERE id = $4`, [newStatus, newAttempts, errorMsg, record.id]);
                if (newStatus === 'failed') {
 ...
 */
export async function cleanupSentNotifications() {
    try {
        const result = await query(`DELETE FROM notification_outbox
       WHERE status = 'sent'
