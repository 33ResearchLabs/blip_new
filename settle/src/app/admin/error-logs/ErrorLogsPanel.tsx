...
/**
 * Pure-content panel for the Error Logs dashboard. Rendered both by the
 * standalone `/admin/error-logs` page and the merged `/admin/observability`
 * tabbed page, so nothing in here assumes a specific outer chrome.
 *
 * Panel-specific action buttons (Test Sentry + Logs, Clear Test Logs,
 * Resolve All Visible, Grouped toggle, Refresh) used to live in the page
 * header. They now render inside the panel so they travel with it.
 */
...
export default function ErrorLogsPanel({ onRefreshStateChange }: ErrorLogsPanelProps) {
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <button
          onClick={async () => {
            ...
            setTestResult({ ok: localOk && sentryOk, message: parts.join('  ·  ') });
            setTimeout(() => {
              fetchLogs();
            }, 800);
            setTesting(false);
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <table className="w-full text-xs">
          ...
          <tbody>
            ...
            {rows.map((r) => {
              const when = grouped ? r.last_seen_at || r.created_at : r.created_at;
