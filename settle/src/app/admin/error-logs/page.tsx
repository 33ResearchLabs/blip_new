...
const TYPE_LABELS: Array<[string, string]> = [
  // Exact types first
  [
    "solana.balance_fetch_failed",
    "Solana balance fetch failed (devnet/RPC issue)",
  ],
  ["solana.airdrop_failed", "Solana devnet airdrop failed"],
  [
    "ui.api_fail.network",
    "Network / connection lost (request never reached the server)",
  ],
  ...
  [
    ...
    "Server unhandled promise rejection (escaped all try/catch)",
  ],
  ["process.uncaught_exception", "Server uncaught exception (Node process)"],
  [
    "blipscan.forward_poll_failed",
    "Blipscan indexer: forward-poll failed (new transactions not indexed this cycle)",
  ],
  [
    "blipscan.backfill_failed",
    "Blipscan indexer: backfill failed (historical transactions not indexed)",
  ],
  ...
  [
    "ratelimit.triggered",
    "Rate limit hit (user/IP exceeded allowed request rate)",
  ],
  [
    "auth.session_revoked_or_expired",
    "Request rejected — session was revoked or expired",
  ],
  [
    "auth.no_active_sessions",
    "Request rejected — actor has no active sessions",
  ],
  ...
  [
    "escrow.state_mismatch",
    "Completed order has no escrow funder (should be impossible)",
  ],
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur border-b border-border">
        <div className="flex flex-col min-h-sceen justify-center items-center">
          <div className="max-w-7xl mx-auto px-4 h-12 flex items-center gap-3">
            <div className="flex items-center gap-2 shrink-0">
              <Radio
                size={14}
                className="text-[var(--color-success)] animate-pulse"
              />
          ...
          <div className="flex items-center  gap-2 shrink-0">
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  if (typeof SentryMod.captureException === "function") {
                    SentryMod.captureException(
                      new Error(
                        `[Admin test] Sentry verification event ${testId}`,
                      ),
                ...
                const parts: string[] = [];
                parts.push(
                  localOk ? "✅ error_logs table" : "❌ error_logs table",
                );
                parts.push(
                  sentryOk
                    ? "✅ Sentry"
                    : "❌ Sentry (SDK init failed — check NEXT_PUBLIC_SENTRY_DSN)",
                );
                setTestResult({
                  ok: localOk && sentryOk,
                  message: parts.join("  ·  "),
            ...
            <button
              onClick={async () => {
                if (
                  !confirm(
                    "Remove all diagnostic / test log entries (test.*, errorTracking.*, manual.*)?",
                  )
                )
                  return;
                try {
            ...
            <button
              onClick={async () => {
                if (
                  !confirm(
                    "Mark ALL currently-visible entries as resolved? (slow queries, heartbeat aborts, etc. — they stay in DB for audit but hide from the list)",
                  )
                )
                  return;
                try {
                  ...
                  setRows([]);
                  alert(
                    `Resolved ${succeeded} ${grouped ? "groups" : "entries"}.`,
                  );
                  fetchLogs();
                } catch (e) {
                  alert((e as Error).message || "Network error");
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="grid grid-cols-4 gap-2 mb-3">
          {(["INFO", "WARN", "ERROR", "CRITICAL"] as const).map((s) => (
            <div
              key={s}
              className={`rounded-lg border px-3 py-2 text-xs ${SEV_STYLES[s]}`}
            >
        ...
        {testResult && (
          ...
          >
            ...
            <div className="text-xs opacity-70">
              Check the row below (type{" "}
              <span className="font-mono">test.admin_button</span>) and your
              <a
            ...
            <tbody>
              {rows.length === 0 && !loading ? (
                <tr>
                  <td
                    colSpan={grouped ? 7 : 6}
                    className="px-3 py-8 text-center text-foreground/40"
                  >
                    No error logs yet.
                  </td>
                </tr>
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        ...
                        title={
                          grouped
                            ? `Resolve all ×${r.occurrence_count ?? 1} occurrences`
                            : "Resolve this error"
                        }
                      >
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-foreground/60 mb-3">
              <div>
                Severity:{" "}
                <span className="text-foreground">{selected.severity}</span>
              </div>
              <div>
                Source:{" "}
              ...
              <div>
                ...
                <span className="text-foreground">
                  {new Date(
                    selected.last_seen_at || selected.created_at,
                  ).toLocaleString()}
                </span>
              </div>
