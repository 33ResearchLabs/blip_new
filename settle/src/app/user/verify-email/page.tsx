...
// falls back to the waitlist sign-in page.
...
function VerifyEmailBody({ nextHref }: { nextHref: string }) {
  ...
  // Guard against React 18 StrictMode double-invocation in dev (otherwise
  // the second call hits an already-used token and flips to "error").
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/auth/user/verify-email?token=${encodeURIComponent(token)}&id=${encodeURIComponent(id)}`,
      ...
      >
        <div className="text-center space-y-2">
          ...
          <p className="text-[12.5px] text-[#6e6e73] leading-relaxed">
            {state === "already"
              ? "Your email is already confirmed. Sending you to sign in…"
        ...
        <EmailFlowAccentPill
          ...
          title="Verified emails protect your account"
          body="Helps us safeguard your funds and restore access if you lose your device."
        />
...
function Inner() {
  ...
  return (
    <EmailFlowShell
      ...
      headlineAccent="unlock your account."
    >
