...
// Reads ?token=xxx&id=xxx from the password-reset email link, then
// POSTs the new password to /api/auth/user/reset-password. Min length
// matches the user register flow (6 chars).
  ...
  if (!token || !userId) {
    return (
      <div className="space-y-5">
        ...
        <EmailFlowPrimaryCta href="/user/forgot-password">Request new link →</EmailFlowPrimaryCta>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="text-center space-y-2">
          ...
          <p className="text-[12.5px] text-[#6e6e73] leading-relaxed">
            Your new password is ready. Sending you to sign in…
          </p>
        </div>
        ...
        <EmailFlowAccentPill
          ...
          body="Sign back in with your new password on every device that needs access."
        />

        <EmailFlowPrimaryCta href="/waitlist/login">Go to sign in →</EmailFlowPrimaryCta>
      </motion.div>
    );
  }
