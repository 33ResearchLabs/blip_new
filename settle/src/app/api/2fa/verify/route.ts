...
import { getTotpStatus, verifyTotpEncrypted, enableTotp, recordAttempt, isRateLimited } from '@/lib/auth/totp';
...
export async function POST(request: NextRequest) {
  try {
    ...
    return successResponse({ enabled: true, message: '2FA has been enabled successfully.' });
  } catch (error) {
