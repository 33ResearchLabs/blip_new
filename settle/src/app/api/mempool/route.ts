...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'accept') {
      ...
      const orderRow = await queryOne<{ merchant_id: string | null }>(
        'SELECT merchant_id FROM orders WHERE id = $1',
        [order_id]
      );
      if (orderRow?.merchant_id === merchant_id) {
    ...
    // Upsert merchant quote (merged from /api/merchant-quotes)
    if (action === 'upsert_quote') {
      const {
      ...
      if (!merchant_id || !min_price_aed_per_usdt || available_liquidity_usdt === undefined) {
        return validationErrorResponse(['merchant_id, min_price_aed_per_usdt, and available_liquidity_usdt are required']);
      }
      ...
      if (min_price_aed_per_usdt <= 0) return validationErrorResponse(['min_price_aed_per_usdt must be positive']);
      if (min_size_usdt <= 0 || max_size_usdt <= 0) return validationErrorResponse(['min_size_usdt and max_size_usdt must be positive']);
      if (min_size_usdt > max_size_usdt) return validationErrorResponse(['min_size_usdt cannot exceed max_size_usdt']);
      if (available_liquidity_usdt < 0) return validationErrorResponse(['available_liquidity_usdt cannot be negative']);
      if (sla_minutes <= 0) return validationErrorResponse(['sla_minutes must be positive']);

      const quote = await upsertMerchantQuote({
        merchant_id, corridor_id, min_price_aed_per_usdt,
        min_size_usdt, max_size_usdt, sla_minutes,
        available_liquidity_usdt, is_online,
      });
