...
import { ISSUE_CATEGORIES, useIssueReporter } from "./useIssueReporter";
import type { AttachmentInput, IssueCategory } from "./types";
...
// Per-file caps and accept lists are static — they apply regardless
// of how many screenshots/attachments the host allows. Count caps live
// on the config object (config.maxAttachments / config.maxScreenshots).
...
export function IssueReporter({
  ...
  /**
   * Per-mount overrides. Each falls back to the value supplied by
   * <IssueReporterProvider config={...}> when omitted, so most hosts
   * never need to pass anything here.
   */
  triggerLabel?: string;
  position?: "bottom-right" | "bottom-left";
  /**
   * Caller-supplied login state. When provided, overrides
   * `config.authed` for this mount only. Lets a single page pass its
   * own auth signal (e.g. a session-only logged-in user) without
   * affecting global config.
   */
  authed?: boolean;
  /**
  ...
  // Resolve all knobs against the host-supplied config, with the
  // per-mount props taking precedence. Defaults are baked into the
  // provider — see ./config.tsx.
  const config = useIssueReporterConfig();
  const resolvedTriggerLabel = triggerLabel ?? config.triggerLabel;
  const resolvedPosition = position ?? config.position;
  const resolvedHideTrigger = hideTrigger ?? config.hideTrigger;
  const authed = authedProp ?? config.authed;
  // Count caps come from config so a host can dial them up/down per
  // app. They keep their original names so the rest of this file
  // (toasts, JSX, handlers) doesn't change.
  const MAX_SCREENSHOTS = config.maxScreenshots;
  ...
  // localStorage key is namespaced by the resolved corner so the user
  // can park the button independently in bottom-left vs bottom-right
  // mounts. Prefix is intentionally generic — not Blip-specific —
  // since the plugin can be reused across apps.
  const DRAG_STORAGE_KEY = `issue-reporter-pos-${resolvedPosition}`;
  ...
  const positionClasses =
    resolvedPosition === "bottom-left" ? "left-4 bottom-4" : "right-4 bottom-4";
  ...
  return (
    <>
      ...
      {!reporter.isOpen && !resolvedHideTrigger && (
        <button
          type="button"
