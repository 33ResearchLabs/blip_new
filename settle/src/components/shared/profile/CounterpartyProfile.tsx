      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pb-4 space-y-3">
        ...
        <div className="flex items-start gap-4 pt-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-500/10 text-amber-600 border border-amber-500/20">
            ...
            <p className="text-[11px] text-text-tertiary mt-2 inline-flex items-center gap-1.5 flex-wrap">
              ...
              {active && (
                <>
                  ...
                  <span className={data.isOnline ? "text-green-500" : ""}>{active}</span>
                </>
              )}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className={`${card} p-5`}>
            ...
            <div className="space-y-2.5">
              ...
              <StatRow icon={<Activity className="w-4 h-4" />} label="Success Rate" value={formatPercentage(data.stats.successRate)} valueClass="text-green-500" />
                ...
                <div key={r.id} className={`rounded-xl p-3.5 border border-border-subtle ${surfaces.inset}`}>
                  <div className="flex items-center gap-0.5 mb-1.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`w-3.5 h-3.5 ${i < Math.round(r.rating) ? "text-green-500 fill-green-500" : "text-text-quaternary"}`}
                      />
                    ))}
                  </div>
...
function VerifRow({ icon, label, ok }: { icon: React.ReactNode; label: string; ok: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${ok ? "bg-green-500/10 text-green-500" : "bg-text-primary/[0.05] text-text-tertiary"}`}>
        {icon}
      </span>
      <span className="text-[13px] text-text-secondary flex-1">{label}</span>
      {ok ? (
        <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-green-500/10 text-green-500 inline-flex items-center gap-1">
...
function RiskStat({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex flex-col items-center text-center gap-1.5">
      <span className="w-8 h-8 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center">{icon}</span>
...
function SocialItem({ icon, label, handle, verified }: { icon: React.ReactNode; label: string; handle: string; verified: boolean }) {
  return (
    <div className="flex flex-col items-center text-center gap-1">
      ...
      {verified && (
        <span className="text-[10px] text-green-500 font-semibold inline-flex items-center gap-0.5">
