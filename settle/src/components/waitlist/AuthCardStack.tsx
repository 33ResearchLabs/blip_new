...
// Phone-frame card stack used on the waitlist login pages. Ported from
// futureStick's blip-protocol-ui/src/components/auth/AuthCardStack.tsx.
...
export default function AuthCardStack({ variant = 'user', className = '' }: AuthCardStackProps) {
  return (
    <div className={`relative w-[460px] h-[420px] ${className}`}>
      {/* Main phone-frame card */}
      <motion.div
      ...
      >
        ...
        <div className="px-4 pt-1 pb-14">
          <div className="flex items-center justify-between mb-2">
            <div>
              <div className="text-[8px] text-black/40 dark:text-white/40">Welcome back</div>
              <div className="text-[11px] font-semibold text-black dark:text-white">Alex 👋</div>
            </div>
            <div className="relative">
              <div className="w-6 h-6 rounded-full bg-black/[0.06] dark:bg-white/[0.08]" />
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-black dark:bg-white rounded-full text-[6px] text-white flex items-center justify-center">2</div>
            </div>
          </div>

          <div className="mb-3">
            <div className="text-[9px] text-black/40 dark:text-white/40 mb-1">Total Balance</div>
            <div className="text-[24px] font-bold text-black dark:text-white leading-none">
              $12,847<span className="text-[12px] text-black/30 dark:text-white/30">.00</span>
            </div>
            <div className="text-[8px] text-green-500 mt-1">▲ +4.2% this week</div>
          </div>

          <div className="grid grid-cols-4 gap-2 mb-3">
            {['Send', 'Receive', 'Swap', 'Earn'].map((action) => (
              <div key={action} className="flex flex-col items-center justify-center py-2 rounded-lg bg-black/[0.04] dark:bg-white/[0.06]">
                <div className="w-4 h-4 rounded-full bg-black/[0.15] mb-1" />
                <div className="text-[7px] text-black/60 dark:text-white/60">{action}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-end gap-[3px] h-10 mb-3">
            {[25, 40, 35, 55, 45, 70, 50, 85, 60, 75, 65, 90].map((h, i) => (
              <div key={i} className="flex-1 rounded-t-[2px]" style={{ height: `${h}%`, background: i >= 9 ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.06)' }} />
            ))}
          </div>

          <div className="mb-3">
            <div className="text-[8px] text-black/40 dark:text-white/40 mb-1">Portfolio</div>
      ...
      >
        <div className="w-9 h-9 rounded-full bg-white dark:bg-[#1a1a1a] border border-black/[0.06] dark:border-white/[0.08] shadow-[0_4px_16px_-4px_rgba(0,0,0,0.12)] flex items-center justify-center text-base">⚡</div>
      </motion.div>
      ...
      >
        <div className="p-2.5">
          <div className="flex items-center gap-2 mb-1.5">
            <div className="w-5 h-5 rounded-lg flex items-center justify-center" style={{ background: variant === 'merchant' ? 'rgba(139, 92, 246, 0.1)' : 'rgba(59, 130, 246, 0.1)' }}>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={variant === 'merchant' ? '#8b5cf6' : '#3b82f6'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
              </svg>
            </div>
            <span className="text-[8px] font-semibold text-black/70 dark:text-white/70">Encrypted</span>
          </div>
          <div className="text-[7px] text-black/40 dark:text-white/40 leading-relaxed">Zero-knowledge proofs protect every transaction</div>
        </div>
      </motion.div>
