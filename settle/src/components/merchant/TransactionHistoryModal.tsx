        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-border-subtle">
            <div>
              ...
              <p className="text-xs text-text-tertiary mt-0.5">Your USDT in and out flow</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={fetchTransactions}
                disabled={isLoading}
                className="p-2 hover:bg-foreground/[0.06] rounded-lg transition-colors disabled:opacity-50"
                title="Refresh"
              >
          ...
          {/* Summary — unified balance card with In/Out split */}
          {summary && (
            <div className="p-4 border-b border-border-subtle">
              <div className="rounded-2xl border border-border-subtle bg-foreground/[0.02] overflow-hidden">
                {/* Balance hero */}
                <div className="p-4 flex items-center justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-[26px] leading-none font-extrabold text-text-primary mt-2 tracking-tight">
                      {formatCrypto(summary.current_balance)}
                      <span className="text-sm text-text-tertiary font-semibold ml-1.5">USDT</span>
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 border-t border-border-subtle divide-x divide-border-subtle">
                  <div className="p-3 flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-[var(--color-success)]/12 flex items-center justify-center shrink-0">
                      <TrendingUp className="w-4 h-4 text-[var(--color-success)]" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[10px] text-text-tertiary">Total In</p>
                      <p className="text-sm font-bold text-[var(--color-success)] truncate">
                        +{formatCrypto(summary.total_credits)}
                      </p>
                    </div>
                  </div>
                  <div className="p-3 flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-[var(--color-error)]/12 flex items-center justify-center shrink-0">
                      <TrendingDown className="w-4 h-4 text-[var(--color-error)]" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[10px] text-text-tertiary">Total Out</p>
                      <p className="text-sm font-bold text-[var(--color-error)] truncate">
                        -{formatCrypto(summary.total_debits)}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isLoading && transactions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                <Loader2 className="w-8 h-8 text-text-secondary animate-spin mb-3" />
                <p className="text-xs text-text-tertiary">Loading transactions...</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <button
                  onClick={fetchTransactions}
                  className="flex items-center gap-2 px-3 py-1.5 bg-foreground/[0.05] hover:bg-foreground/[0.08] text-text-secondary rounded-lg transition-colors"
                >
              ...
              <div className="flex flex-col items-center justify-center py-12">
                <div className="w-12 h-12 rounded-full bg-foreground/[0.05] flex items-center justify-center mb-3">
                  <DollarSign className="w-6 h-6 text-text-tertiary" />
                </div>
                <p className="text-xs text-text-tertiary mb-1">No transactions yet</p>
                <p className="text-[10px] text-text-quaternary">Complete a trade to see your history</p>
              </div>
            ) : (
              <div className="space-y-1.5">
                {transactions.map((tx) => (
                  <motion.div
                    ...
                    className="flex items-start gap-2.5 p-3 bg-foreground/[0.02] hover:bg-foreground/[0.05] rounded-xl border border-border-subtle transition-all group"
                  >
                    {/* Icon */}
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                        tx.amount > 0
                          ? 'bg-[var(--color-success)]/12 group-hover:bg-[var(--color-success)]/20'
                          : 'bg-[var(--color-error)]/10 group-hover:bg-[var(--color-error)]/20'
                      } transition-colors`}
                    >
                      {tx.amount > 0 ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[10px] text-text-tertiary mt-0.5 line-clamp-1">{tx.description}</p>
                        </div>
                        <div className="text-right shrink-0">
                          ...
                          >
                            {tx.amount > 0 ? '+' : ''}
                            {formatCrypto(Math.abs(tx.amount))} USDT
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-[10px] text-text-tertiary">{formatDate(tx.created_at)}</p>
                        {tx.order_id && (
                          <>
                            <span className="text-text-quaternary">·</span>
                            <p className="text-[10px] text-text-tertiary font-mono">#{tx.order_id.slice(0, 8)}</p>
                          </>
                        )}
                      </div>
                    </div>
          ...
          <div className="p-4 border-t border-border-subtle">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={onClose}
                className="px-4 py-1.5 bg-foreground/[0.05] hover:bg-foreground/[0.08] text-text-primary rounded-lg transition-colors text-xs font-medium"
              >
                Close
              </button>
            </div>
