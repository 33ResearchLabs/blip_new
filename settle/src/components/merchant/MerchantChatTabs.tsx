...
export function MerchantChatTabs({
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header — hidden on mobile (hideHeading) so the only-on-screen panel
          doesn't redundantly label itself "MESSAGES". */}
      {!hideHeading && (
        <div className="px-3 py-2 border-b border-white/[0.04]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <MessageCircle className="w-3.5 h-3.5 text-white/30" />
              <h2 className="text-[9px] font-bold text-foreground font-mono tracking-wider uppercase">
                Messages
              </h2>
            </div>
            <div className="flex items-center gap-1">
              {totalUnread + disputeUnread > 0 && (
                <span className="text-[9px] border border-primary/30 text-primary px-1.5 py-0.5 rounded-full font-mono tabular-nums">
                ...
                <div className="group relative flex items-center">
                  ...
                  <span className="pointer-events-none absolute top-full right-0 mt-1.5 w-[180px] z-50 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="block rounded-lg bg-foreground text-background text-[10.5px] font-medium px-2.5 py-1.5 leading-snug shadow-xl shadow-black/40">
                      ...
                      still scroll back to read the messages — nothing is
                      deleted.
                    </span>
                  </span>
                </div>
      ...
      {/* Tab strip — Active Chat / Inbox / Disputes. Pinned at the top of
