...
export interface ProfileScreenProps {
  ...
  embeddedWallet?: { state: 'none' | 'locked' | 'unlocked' } | undefined;
  copied: boolean;
...
export const ProfileScreen = ({
  ...
  const shortAddr = solanaWallet.walletAddress
    ? `${solanaWallet.walletAddress.slice(0, 6)}...${solanaWallet.walletAddress.slice(-4)}`
    : null;

  const tradingScore = completedOrders.length > 0
    ? (completedOrders.length / (completedOrders.length + timedOutOrders.length) * 100).toFixed(0) + "%"
    : "—";

  const tier = completedOrders.length >= 50 ? "Elite Trader"
    : completedOrders.length >= 20 ? "Trusted"
    : completedOrders.length >= 10 ? "Established"
    : completedOrders.length >= 3 ? "Emerging"
    : "New Trader";

  const tierLevel = completedOrders.length >= 50 ? 5
    : completedOrders.length >= 20 ? 4
    : completedOrders.length >= 10 ? 3
    : completedOrders.length >= 3 ? 2 : 1;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.3)', fontFamily: 'monospace', marginTop: 2 }}>
                {shortAddr ?? 'Wallet not connected'}
              </p>
            </div>
          </div>
          {solanaWallet.connected && solanaWallet.walletAddress && (
            <motion.button whileTap={{ scale: 0.85 }}
              onClick={async () => { await copyToClipboard(solanaWallet.walletAddress!); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      ...
      <div className="flex-1 px-5 pb-24 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {[
            ...
            { label: 'Volume', value: completedOrders.reduce((s, o) => s + parseFloat(o.cryptoAmount), 0).toFixed(0) + 'U' },
            { label: 'Score', value: tradingScore },
          ].map(stat => (
        ...
        {completedOrders.length > 0 && (
          <div className="rounded-[18px] px-4 py-3 flex items-center justify-between mb-3" style={card}>
            ...
            <div className="flex items-end gap-1">
              {[...Array(5)].map((_, i) => (
                <div key={i} style={{ width: 4, borderRadius: 2, height: 8 + i * 4, background: i < tierLevel ? '#000' : 'rgba(0,0,0,0.12)' }} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="rounded-[18px] p-4 mb-3" style={card}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <p style={{ ...cardLabel, marginBottom: 2 }}>{solanaWallet.connected ? 'Solana Devnet' : 'Not Connected'}</p>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#000', fontFamily: 'monospace' }}>{shortAddr ?? 'Connect your wallet'}</p>
            </div>
          </div>
          {solanaWallet.connected && (
            <>
              <div className="flex gap-2 mb-2 pt-3" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
                {[
                  { label: 'SOL', value: solanaWallet.solBalance?.toFixed(4) ?? '—' },
                  { label: 'USDT', value: solanaWallet.usdtBalance?.toFixed(2) ?? '—' },
                ].map(b => (
          ...
          {!solanaWallet.connected && (
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => {
              if (embeddedWallet) {
                if (embeddedWallet.state === 'none') setShowWalletSetup?.(true);
                else if (embeddedWallet.state === 'locked') setShowWalletUnlock?.(true);
              } else {
        ...
        <div className="flex flex-col gap-2 mb-3">
          {bankAccounts.length === 0 && (
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.2)', textAlign: 'center', padding: '10px 0' }}>No bank accounts added</p>
          )}
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span style={{
                        ...
                        {dispute.resolvedInFavorOf === 'user' ? 'Won' : dispute.resolvedInFavorOf === 'merchant' ? 'Lost' : 'Split'}
                      </span>
                    </div>
                    <p style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)' }}>{new Date(dispute.resolvedAt).toLocaleDateString()}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.35)' }}>vs {dispute.otherPartyName}</p>
                    <p style={{ fontSize: 14, fontWeight: 800, color: '#000', letterSpacing: '-0.01em' }}>${dispute.cryptoAmount.toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {showAddBank && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/60 z-40" onClick={() => setShowAddBank(false)} />
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              transition={{ type: 'spring', damping: 30 }}
              className="fixed bottom-32 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm rounded-3xl"
              style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.06)' }}>
              ...
              <div className="px-5 py-4 flex flex-col gap-3">
                {[
                  { label: 'Bank Name', key: 'bank' as const, placeholder: 'Emirates NBD' },
                  { label: 'IBAN', key: 'iban' as const, placeholder: 'AE12 0345 0000 0012 3456 789' },
                  { label: 'Account Name', key: 'name' as const, placeholder: 'John Doe' },
                ].map(field => (
                  <div key={field.key}>
                    <p style={{ ...cardLabel, marginBottom: 6, display: 'block' }}>{field.label}</p>
                    <input value={newBank[field.key]}
                      onChange={e => setNewBank(p => ({ ...p, [field.key]: e.target.value }))}
              ...
              <div className="px-5 pb-8 pt-1">
                <motion.button whileTap={{ scale: 0.97 }} onClick={addBankAccount}
