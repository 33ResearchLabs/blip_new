  ...
  return (
    <div className="relative flex flex-col" style={{ minHeight: '100dvh', background: '#ffffff' }}>
      ...
      <div className="relative shrink-0" style={{
        ...
        /* Wallet/card illusion gradient — diagonal sweep like a premium physical card */
        background: `#000000
        `,
      }}>
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-b-[40px]">
          ...
          {/* Diagonal card sheen — top-left to bottom-right, like light catching a card */}
          <div style={{
            position: 'absolute', inset: 0,
            background: 'linear-gradient(135deg, rgba(255,255,255,0.028) 0%, transparent 45%, rgba(255,255,255,0.012) 100%)',
          }} />

          {/* Top-left corner bloom — deep indigo, like a holographic card corner */}
          <div style={{
          ...
          {/* Bottom-right bloom — deep blue, opposite corner */}
          <div style={{
          ...
          {/* Animated center shimmer — slow sweep like light reflecting off a card */}
          <motion.div
            animate={{ x: ['-100%', '200%'] }}
          ...
          {/* Horizontal rule near the bottom — fades out */}
          <div style={{
      ...
      {/* ══════════════════════════════════════════════
          WHITE CARD — elevated from below, IS the card
         ══════════════════════════════════════════════ */}
      <motion.div
      ...
      >
        <div className={`${maxW} mx-auto px-5 pt-4`}>
          ...
          {/* ── Promo banner — white background, clean ── */}
          {/* {(() => {
            const banners = [
              {
                tag: 'NEW', tagColor: '#3b82f6',
                headline: 'Merchant Dashboard',
                sub: 'Go live and earn on every trade',
                screen: 'profile' as Screen,
              },
              {
                tag: 'LIMITED', tagColor: '#0ea5e9',
                headline: '$100 Airdrop',
                sub: 'Complete your first trade to claim',
                screen: 'trade' as Screen,
              },
            ];
            const banner = banners[Math.floor(Date.now() / 86400000) % banners.length];
            return (
              <motion.button
                initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.22, duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                whileTap={{ scale: 0.97 }}
                onClick={() => setScreen(banner.screen)}
                className="w-full text-left flex items-center justify-between relative overflow-hidden"
                style={{
                  marginBottom: 20,
                  height: 110,
                  borderRadius: 26,
                  background: '#000',
                  border: `1px solid ${banner.tagColor}33`,
              ...
              >
                ...
                <div style={{
                  ...
                  boxShadow: `0 0 8px ${banner.tagColor}`,
                }} />

                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <motion.div animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1.6, repeat: Infinity }}
                      style={{ width: 5, height: 5, borderRadius: '50%', background: banner.tagColor, boxShadow: `0 0 6px ${banner.tagColor}` }} />
                    <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.2em', color: banner.tagColor, textTransform: 'uppercase' }}>
                      {banner.tag}
                    </span>
                  </div>
                  <p style={{ fontSize: 19, fontWeight: 800, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: 4, fontFamily: 'var(--font-geist-sans), -apple-system, system-ui, sans-serif' }}>
                    {banner.headline}
                  </p>
                  <p style={{ fontSize: 11, fontWeight: 500, color: 'rgba(255,255,255,0.35)', fontFamily: 'var(--font-geist-sans), -apple-system, system-ui, sans-serif' }}>{banner.sub}</p>
                </div>

                <div className="shrink-0 flex items-center justify-center rounded-2xl"
                  style={{ padding: '12px 20px', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,0.25)' }}>
                  <span style={{ fontSize: 16, fontWeight: 800, color: '#000', letterSpacing: '-0.02em' }}>JOIN</span>
                </div>
              </motion.button>
            );
          })()} */}
                  ...
                  return (
                    <motion.div key={order.merchant.id}
                      ...
                      <div className="relative">
                        {/* Ring — black for active, light grey for inactive */}
                        <div style={{
                          padding: 2, borderRadius: 22,
                          background: isActive ? '#000' : 'transparent',
                          border: isActive ? 'none' : '1.5px solid #e5e5e5',
                        }}>
                          {/* Avatar — black bg, white initial */}
