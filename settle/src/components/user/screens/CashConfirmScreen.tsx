    ...
    <>
      ...
      <div className="flex-1 px-5 overflow-auto">
        ...
        <div className={`rounded-2xl p-4 mb-4 ${CARD}`}>
          <div className="flex items-center justify-between mb-3">
            <span className="text-[13px] text-text-tertiary">You {tradeType === "buy" ? "pay" : "receive"}</span>
            <span className="text-[22px] font-semibold text-text-primary">{'\u062F.\u0625'} {parseFloat(fiatAmount).toLocaleString()}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] text-text-secondary">{selectedOffer.merchant.total_trades} trades</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative h-36">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex flex-col items-center">
                <div className="w-12 h-12 rounded-full flex items-center justify-center shadow-lg mb-1 bg-border-medium">
                  <MapPin className="w-6 h-6 text-text-primary" />
                </div>
                <div className="w-1 h-4 rounded-b-full bg-border-medium" />
              </div>
            </div>
            ...
            {selectedOffer.location_lat && selectedOffer.location_lng && (
              ...
              >
                ...
                <span className="text-[12px] font-medium text-text-primary">Open Maps</span>
              </button>
            )}
          </div>
          <div className="p-4 space-y-3">
            <div>
              <p className="text-[15px] font-medium text-text-primary">{selectedOffer.location_name}</p>
              <p className="text-[13px] text-text-secondary">{selectedOffer.location_address}</p>
            </div>
            {selectedOffer.meeting_instructions && (
              <div className="pt-3 border-t border-border-subtle">
                <p className="text-[11px] uppercase tracking-wide mb-1 text-text-tertiary">Meeting spot</p>
                <div className="flex items-start gap-2">
                  <Navigation className="w-4 h-4 flex-shrink-0 mt-0.5 text-text-primary" />
                  <p className="text-[13px] text-text-primary">{selectedOffer.meeting_instructions}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className={`rounded-2xl p-4 mb-4 ${CARD}`}>
          <div className="flex items-start gap-3">
            <Shield className="w-5 h-5 flex-shrink-0 mt-0.5 text-text-secondary" />
            <div>
              <p className="text-[13px] font-medium mb-1 text-text-secondary">Safety tips</p>
              <ul className="text-[12px] space-y-1 text-text-secondary">
      ...
      <div className="px-5 pb-10 space-y-3">
        <motion.button
          ...
          disabled={isLoading}
          className="w-full py-4 rounded-2xl text-[17px] font-semibold flex items-center justify-center gap-2 bg-accent text-accent-text"
        >
          {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Confirm & Start Trade"}
        </motion.button>
        <button
          onClick={() => { setScreen("home"); setSelectedOffer(null); }}
          className="w-full py-3 text-[15px] font-medium text-text-tertiary"
        >
          Cancel
        </button>
      </div>
