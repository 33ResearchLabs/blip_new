...
export function UserWelcomePage({ onGetStarted, onSignIn }: UserWelcomePageProps) {
  ...
  return (
    <div className="flex-1 w-full max-w-7xl mx-auto flex flex-col bg-surface-base text-text-primary overflow-y-auto relative">
      {/* Ambient glow */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] md:w-[700px] h-[300px] md:h-[450px] bg-warning/[0.04] rounded-full blur-[120px]" />
      </div>

      <div className="relative z-10 flex flex-col px-5 md:px-8 lg:px-12 pb-10">

        {/* ─── TOP BAR (desktop) ─────────────────────────── */}
        <div className="hidden md:flex items-center justify-between pt-6 pb-2">
          <div className="flex items-center gap-2">
            <Zap className="w-6 h-6 text-text-primary fill-current" />
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onSignIn}
              className="px-4 py-2 rounded-lg text-[13px] font-semibold text-text-secondary hover:text-text-primary transition-colors"
            >
              Sign In
            </button>
            <button
              onClick={onGetStarted}
              className="px-4 py-2 rounded-lg text-[13px] font-bold bg-accent text-accent-text hover:opacity-90 transition-all"
            >
              Get Started
            </button>
          </div>
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center pt-10 md:pt-12 lg:pt-16 pb-8"
        >
          {/* Left — title + role chooser */}
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left max-w-[520px] mx-auto lg:mx-0">
            ...
            <motion.h1
              variants={fadeUp}
              className="text-2xl md:text-4xl lg:text-5xl font-extrabold text-text-primary leading-tight tracking-tight mt-4 md:mt-0 mb-3 md:mb-4"
            >
            ...
            <motion.p
              variants={fadeUp}
              className="text-[13px] md:text-[15px] text-text-secondary leading-relaxed max-w-[420px] mb-7 md:mb-8"
            >
              Exchange fiat for crypto with verified merchants. Zero fees, escrow protection, real-time settlement.
            </motion.p>

            {/* Role chooser — main entry point */}
            <motion.div variants={fadeUp} className="w-full">
              <p className={`${SECTION_LABEL} mb-3 text-center lg:text-left`}>Continue As</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                <button
                  onClick={onSignIn}
                  className="group relative flex items-start gap-3 p-4 rounded-2xl bg-surface-card border border-border-subtle hover:border-text-secondary transition-all text-left"
                >
                  <div className="w-10 h-10 rounded-xl bg-surface-hover flex items-center justify-center flex-shrink-0 group-hover:bg-accent/10 transition-colors">
                    <UserIcon className="w-5 h-5 text-text-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[14px] font-bold text-text-primary">User</p>
                    ...
                    <p className="text-[11px] text-text-tertiary leading-snug mt-0.5">
                      Buy & sell crypto with verified merchants
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[14px] font-bold text-text-primary">Merchant</p>
                      <ChevronRight className="w-4 h-4 text-text-tertiary group-hover:text-text-primary transition-colors" />
                    </div>
                    <p className="text-[11px] text-text-tertiary leading-snug mt-0.5">
                      Run a business, accept orders, earn rewards
                    </p>
                  </div>
                </button>
              </div>
              ...
              <div className="mt-4 flex flex-col sm:flex-row gap-2.5">
                <button
                  onClick={onGetStarted}
                  className="flex-1 py-3 bg-accent text-accent-text rounded-xl font-bold text-sm hover:opacity-90 transition-all flex items-center justify-center gap-2"
                >
          ...
          {/* Right — phone mockup (centered on mobile, right on desktop) */}
          <motion.div
          ...
          >
            <div className="relative w-full" style={{ maxWidth: 340 }}>
              <p className={`${SECTION_LABEL} mb-4 text-center lg:hidden`}>See How It Works</p>
        ...
        >
          {[
            ...
            <div key={i} className="text-center py-4 rounded-xl bg-surface-card border border-border-subtle">
              <div className="text-base md:text-xl font-bold text-text-primary font-mono">{s.value}</div>
              <div className="text-[10px] text-text-tertiary uppercase tracking-wider mt-1">{s.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        {/* ─── FEATURES ──────────────────────────────────── */}
        <div className="mb-10">
          <h3 className={`${SECTION_LABEL} mb-4`}>Why Blip</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {features.map((f, i) => (
              <motion.div
                ...
                className="p-4 rounded-xl bg-surface-card border border-border-subtle"
              >
                <div className="w-10 h-10 rounded-lg bg-surface-hover flex items-center justify-center text-text-tertiary mb-3">
                  {f.icon}
                </div>
                <div className="text-[13px] md:text-sm font-semibold text-text-primary mb-1">{f.title}</div>
                <div className="text-[11px] md:text-[12px] text-text-tertiary leading-relaxed">{f.desc}</div>
              </motion.div>
            ))}
          </div>
        </div>

        {/* ─── TRUST + BOTTOM CTA — side by side on desktop ── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 1.1 }}
            className="p-5 md:p-6 rounded-2xl border border-border-subtle"
          ...
          >
            ...
            <h4 className="text-base md:text-lg font-bold text-text-primary mb-2">Every Trade Is Protected</h4>
            <p className="text-[11px] md:text-[12px] text-text-secondary leading-relaxed mb-4">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <span className="text-[11px] md:text-[12px] text-text-secondary">{item.text}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="p-6 md:p-8 rounded-2xl bg-accent text-center relative overflow-hidden flex flex-col justify-center"
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.08),transparent)]" />
            <h4 className="text-lg md:text-2xl font-extrabold text-accent-text mb-2 relative z-10">Ready to Trade?</h4>
            <p className="text-[11px] md:text-[13px] text-accent-text/60 mb-4 relative z-10">Create a free account in seconds</p>
            <div className="flex flex-col sm:flex-row gap-2 relative z-10">
              <button
                onClick={onGetStarted}
                className="flex-1 py-3 bg-surface-base text-text-primary rounded-xl font-bold text-sm hover:opacity-90 transition-all"
              >
                Create Free Account
              </button>
              <button
                onClick={goToMerchant}
                className="flex-1 py-3 bg-surface-base/20 hover:bg-surface-base/30 text-accent-text border border-accent-text/20 rounded-xl font-bold text-sm transition-all"
              >
                I&apos;m a Merchant
              </button>
            </div>
