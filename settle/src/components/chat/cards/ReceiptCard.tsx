...
export function ReceiptCard({ data, currentStatus, theme = 'dark' }: ReceiptCardProps) {
  ...
  const t = {
    ...
    subtitle: isLight ? 'text-black/65' : 'text-white/50',
    label: isLight ? 'text-black/60' : 'text-white/50',
    value: isLight ? 'text-black' : 'text-white',
    valueDim: isLight ? 'text-black/80' : 'text-white/70',
    icon: isLight ? 'text-black/55' : 'text-white/40',
    iconDim: isLight ? 'text-black/30' : 'text-white/20',
    ...
    timestamp: isLight ? 'text-black/50' : 'text-white/30',
    timestampVal: isLight ? 'text-black/70' : 'text-white/50',
      ...
      <div className={`px-4 py-3 space-y-2.5 border-b ${t.divider}`}>
        <div className="flex justify-between items-center">
          ...
          <span className={`text-[15px] ${t.value} font-bold font-mono tabular-nums`}>
            {formatAmount(data.crypto_amount)} {data.crypto_currency || 'USDT'}
          </span>
        </div>
