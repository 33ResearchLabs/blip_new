...
interface ChatRoomProps {
  ...
  onSendMessage: (text: string, imageUrl?: string, fileData?: {
    fileUrl: string;
    fileName: string;
    fileSize: number;
    mimeType: string;
...
const ROLE_COLORS = {
  user: { bg: 'bg-blue-500/20', border: 'border-blue-500/30', text: 'text-blue-400', name: 'text-blue-400' },
  merchant: { bg: 'bg-orange-500/20', border: 'border-orange-500/30', text: 'text-orange-400', name: 'text-orange-400' },
  compliance: { bg: 'bg-red-500/20', border: 'border-red-500/30', text: 'text-red-400', name: 'text-red-400' },
  system: { bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-400', name: 'text-amber-400' },
};
...
function SenderAvatar({ senderType, senderName }: { senderType?: string; senderName?: string }) {
  switch (senderType) {
    case 'merchant':
      return (
...
function ImagePreview({ imageUrl, caption }: { imageUrl: string; caption?: string }) {
  ...
  return (
    <>
      <div className="max-w-[220px] rounded-lg overflow-hidden cursor-pointer" onClick={() => setLightbox(true)}>
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4" onClick={() => setLightbox(false)}>
          <button onClick={() => setLightbox(false)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20">
            <X className="w-6 h-6 text-white" />
          </button>
          <div className="absolute top-4 left-4 flex gap-2">
            <button onClick={(e) => { e.stopPropagation(); handleDownload(); }} className="p-2 rounded-full bg-white/10 hover:bg-white/20" title="Download">
              <Download className="w-5 h-5 text-white" />
            </button>
            <a href={imageUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 rounded-full bg-white/10 hover:bg-white/20" title="Open">
              <ExternalLink className="w-5 h-5 text-white" />
            </a>
          </div>
          <img src={imageUrl} alt="Full size" className="max-w-full max-h-full object-contain rounded-lg" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </>
  );
...
function FileAttachment({ fileName, fileSize, fileUrl, mimeType }: { fileName?: string | null; fileSize?: number | null; fileUrl?: string | null; mimeType?: string | null }) {
  const getIcon = () => {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-white/[0.04] rounded-lg border border-white/[0.06] max-w-[250px]">
      ...
      <div className="flex-1 min-w-0">
        ...
        {fileSize && <p className="text-[10px] text-gray-500">{formatFileSize(fileSize)}</p>}
      </div>
      {fileUrl && (
        <a
          href={fileUrl}
...
export function ChatRoom({
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const category: 'image' | 'document' = file.type.startsWith('image/') ? 'image' : 'document';
  ...
  const uploadAndSend = useCallback(async () => {
    ...
    try {
      ...
      if (fileUrl) {
        const text = messageText.trim() || (isImage ? 'Photo' : pendingFile.file.name);
  ...
  const getDisplayName = (msg: ChatMessage): string => {
    ...
    switch (msg.senderType) {
      case 'user': return userName || 'Buyer';
      case 'merchant': return merchantName || 'Seller';
      case 'compliance': return complianceName || 'Compliance Officer';
      default: return 'System';
    }
  };
  ...
  const getTypingLabel = (): string => {
    ...
    switch (typingActorType) {
      case 'user': return `${userName || 'Buyer'} is typing...`;
      case 'merchant': return `${merchantName || 'Seller'} is typing...`;
      case 'compliance': return `${complianceName || 'Compliance'} is typing...`;
      default: return 'Someone is typing...';
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      {/* Presence bar */}
      {presence.length > 0 && (
        <div className="px-3 py-1.5 border-b border-white/[0.04] bg-[#0d0d0d]/50 flex items-center gap-3">
          {presence.map((member) => (
            <div key={`${member.actorType}:${member.actorId}`} className="flex items-center gap-1.5">
              <div className="relative">
      ...
      {isFrozen && (
        <div className="px-3 py-2 bg-red-500/10 border-b border-red-500/20 flex items-center gap-2">
          ...
          <span className="text-xs text-red-400">Chat frozen by compliance officer</span>
      ...
      {/* Messages area */}
      <div className="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
        {isLoading && (
        ...
        {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className="flex justify-center my-2">
                <div className="px-3 py-1.5 bg-white/[0.03] rounded-full border border-white/[0.04]">
                  <p className="text-[11px] text-gray-500 text-center">{msg.text}</p>
                </div>
              </div>
            );
          }
          ...
          const roleColor = ROLE_COLORS[msg.senderType || 'user'] || ROLE_COLORS.user;

          return (
            <div
              key={msg.id}
              className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'} ${msg.isHighlighted ? 'ring-1 ring-yellow-500/50 rounded-lg p-1 bg-yellow-500/5' : ''}`}
            >
              {/* Avatar */}
              ...
              <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                {!isMe && (
                  <div className="flex items-center gap-1.5 mb-0.5 px-1">
                    <span className={`text-[11px] font-medium ${roleColor.name}`}>{displayName}</span>
                    <span className={`text-[9px] px-1 py-0.5 rounded ${roleColor.bg} ${roleColor.text}`}>
                ...
                {currentUserType === 'compliance' && onHighlightMessage && !isMe && (msg.from as string) !== 'system' && (
                  <button
                    ...
                    title={msg.isHighlighted ? 'Remove highlight' : 'Highlight for investigation'}
                  >
          ...
          <div className="flex items-center gap-2 px-2">
            ...
            <div className="px-3 py-2 bg-white/[0.03] rounded-2xl border border-white/[0.04]">
              <div className="flex items-center gap-1">
                <span className="text-[11px] text-gray-500">{getTypingLabel()}</span>
        ...
        <div className="px-3 py-2 border-t border-white/[0.04] bg-[#0d0d0d]">
          <div className="flex items-center gap-3">
            {pendingFile.category === 'image' && pendingFile.previewUrl ? (
              <img src={pendingFile.previewUrl} alt="Preview" className="w-16 h-16 object-cover rounded-lg" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-white/80 truncate">{pendingFile.file.name}</p>
            ...
            {!isUploading && (
              <button onClick={clearPendingFile} className="p-1 hover:bg-white/10 rounded-full transition-colors">
      ...
      <div className="px-3 py-2 border-t border-white/[0.04] bg-[#0d0d0d]">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="hidden"
            disabled={disabled || isUploading || (isFrozen && currentUserType !== 'compliance')}
          />
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={disabled || isUploading || (isFrozen && currentUserType !== 'compliance')}
            className="p-2 rounded-lg hover:bg-white/[0.06] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            title="Attach file"
          >
          ...
          <input
            ...
            placeholder={isFrozen && currentUserType !== 'compliance' ? 'Chat is frozen...' : 'Type a message...'}
            disabled={disabled || isUploading || (isFrozen && currentUserType !== 'compliance')}
            className="flex-1 bg-white/[0.04] rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-gray-600 outline-none focus:ring-1 focus:ring-orange-500/30 disabled:opacity-40"
          />

          {/* Send button */}
          <button
            onClick={handleSend}
            disabled={disabled || isUploading || (isFrozen && currentUserType !== 'compliance') || (!messageText.trim() && !pendingFile)}
            className="p-2.5 rounded-xl bg-orange-500 hover:bg-orange-400 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {isUploading ? (
