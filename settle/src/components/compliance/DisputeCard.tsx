      ...
      <div className="flex items-center gap-1.5 flex-1">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-1">
            ...
            <span className={`text-[8px] px-1 py-px rounded ${seller.color} bg-card font-semibold`}>
              {seller.label}
            </span>
          </div>
          <p className="text-[9px] text-muted">{seller.trades} trades · Locks crypto</p>
        </div>
      </div>
      <span className="text-[10px] text-[var(--color-text-quaternary)]">vs</span>
      <div className="flex items-center gap-1.5 flex-1 justify-end">
        <div className="min-w-0 text-right">
          <div className="flex items-center gap-1 justify-end">
            <span className={`text-[8px] px-1 py-px rounded ${buyer.color} bg-card font-semibold`}>
              {buyer.label}
            </span>
            <p className="text-[10px] font-medium truncate">{buyer.name}</p>
          </div>
          <p className="text-[9px] text-muted">Sends fiat · {buyer.trades} trades</p>
        </div>
...
export default function DisputeCard({
  ...
  if (variant === "open") {
    return (
      <motion.div
        ...
        className="p-3 bg-muted-bg rounded-xl border border-red-500/20 hover:border-red-500/40 transition-all group"
      >
        {/* Header */}
        ...
        <div className="mt-3 flex items-center gap-2">
          <motion.button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-primary/10 hover:bg-primary/20 border border-primary/30 rounded-lg text-xs font-medium text-primary transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-card hover:bg-[var(--color-border-medium)] border border-[var(--color-border-medium)] rounded-lg text-xs text-[var(--color-text-secondary)] transition-all"
          >
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 10 }}
      transition={{ delay: index * 0.03 }}
      className="p-3 bg-muted-bg rounded-xl border border-primary/20 hover:border-primary/30 transition-all"
    >
      {/* Header */}
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold">#{dispute.orderNumber}</span>
            <span className="text-[10px] px-1.5 py-0.5 bg-primary/20 text-primary rounded">In Progress</span>
          </div>
