...
import { validateTransition, shouldRestoreLiquidity, getTransitionEventType, } from '../../orders/stateMachine';
import { normalizeStatus, isTransientStatus, } from '../../orders/statusNormalizer';
...
export async function getOrderWithRelations(id) {
    return queryOne(`SELECT o.*,
            json_build_object(
              'id', u.id,
              'name', u.username,
 ...
 */
export async function getAllPendingOrdersForMerchant(merchantId, status) {
    // is_my_order logic:
...
export async function createOrder(data) {
    return transaction(async (client) => {
       ...
       RETURNING *`, [
        ...
        await client.query(`INSERT INTO order_events (order_id, event_type, actor_type, actor_id, new_status)
       VALUES ($1, 'order_created', 'user', $2, 'pending')`, [order.id, data.user_id]);
        // Reserve liquidity
        await client.query('UPDATE merchant_offers SET available_amount = available_amount - $1 WHERE id = $2', [data.crypto_amount, data.offer_id]);
        return order;
    });
}
/**
 ...
 */
function getStatusChangeMessage(newStatus, order, actorType, metadata) {
 ...
 */
export async function updateOrderStatus(orderId, newStatus, actorType, actorId, metadata) {
    try {
        ...
        return await transaction(async (client) => {
            ...
            const currentResult = await client.query('SELECT * FROM orders WHERE id = $1 FOR UPDATE', [orderId]);
            ...
            const isMerchantClaiming = actorType === 'merchant' &&
            ...
            const isM2MAcceptance = actorType === 'merchant' &&
            ...
            const updateParams = [effectiveStatus, orderId];
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
            ...
            if (newStatus === 'completed') {
                ...
                try {
                    await recordReputationEvent(currentOrder.user_id, 'user', 'order_completed', `Completed order for ${currentOrder.fiat_amount} ${currentOrder.fiat_currency}`, { orderId, amount: currentOrder.fiat_amount, currency: currentOrder.fiat_currency });
                    await recordReputationEvent(currentOrder.merchant_id, 'merchant', 'order_completed', `Completed order for ${currentOrder.fiat_amount} ${currentOrder.fiat_currency}`, { orderId, amount: currentOrder.fiat_amount, currency: currentOrder.fiat_currency });
                }
                catch (repErr) {
            ...
            if (newStatus === 'cancelled') {
                try {
                    await recordReputationEvent(currentOrder.user_id, 'user', 'order_cancelled', `Order cancelled`, { orderId, cancelledBy: actorType });
                    await recordReputationEvent(currentOrder.merchant_id, 'merchant', 'order_cancelled', `Order cancelled`, { orderId, cancelledBy: actorType });
                }
                catch (repErr) {
            ...
            try {
                ...
                if (statusMessage) {
                    await client.query(`INSERT INTO chat_messages (order_id, sender_type, sender_id, content, message_type)
             VALUES ($1, 'system', $2, $3, 'system')`, [orderId, orderId, statusMessage]);
                }
                ...
                if (effectiveStatus === 'accepted' && updatedOrder.payment_method === 'bank') {
                    const paymentDetails = updatedOrder.payment_details;
                    if (paymentDetails) {
                        ...
                        await client.query(`INSERT INTO chat_messages (order_id, sender_type, sender_id, content, message_type)
               VALUES ($1, 'system', $2, $3, 'system')`, [orderId, orderId, bankInfoMessage]);
                    }
                }
                ...
                if (effectiveStatus === 'escrowed' && updatedOrder.escrow_tx_hash) {
                    ...
                    await client.query(`INSERT INTO chat_messages (order_id, sender_type, sender_id, content, message_type)
             VALUES ($1, 'system', $2, $3, 'system')`, [orderId, orderId, escrowInfoMessage]);
                }
                ...
                if (effectiveStatus === 'completed' && updatedOrder.release_tx_hash) {
                    ...
                    await client.query(`INSERT INTO chat_messages (order_id, sender_type, sender_id, content, message_type)
             VALUES ($1, 'system', $2, $3, 'system')`, [orderId, orderId, releaseInfoMessage]);
                }
            }
            catch (msgErr) {
 ...
 */
export async function updateOrderStatusLegacy(orderId, newStatus, actorType, actorId, metadata) {
    const result = await updateOrderStatus(orderId, newStatus, actorType, actorId, metadata);
    return result.success ? result.order : null;
}
export async function cancelOrder(orderId, actorType, actorId, reason) {
    return updateOrderStatus(orderId, 'cancelled', actorType, actorId, { reason });
}
// Order Events
export async function getOrderEvents(orderId) {
    return query('SELECT * FROM order_events WHERE order_id = $1 ORDER BY created_at ASC', [orderId]);
}
// Chat Messages
export async function getOrderMessages(orderId) {
    ...
    WHERE cm.order_id = $1
      AND cm.sender_type != 'system'
      AND cm.message_type != 'system'
    ORDER BY cm.created_at ASC`, [orderId]);
}
export async function sendMessage(data) {
    const result = await queryOne(`INSERT INTO chat_messages (order_id, sender_type, sender_id, content, message_type, image_url)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`, [
        data.order_id,
        data.sender_type,
...
export async function markMessagesAsRead(orderId, readerType) {
    // Mark messages as read that were NOT sent by the reader
    await query(`UPDATE chat_messages
     SET is_read = true, read_at = NOW()
     WHERE order_id = $1 AND sender_type != $2 AND is_read = false`, [orderId, readerType]);
}
// Update has_manual_message flag when user/merchant sends a message
export async function markOrderHasManualMessage(orderId) {
    await query(`UPDATE orders SET has_manual_message = true WHERE id = $1 AND has_manual_message = false`, [orderId]);
}
...
export async function expireOldOrders() {
    ...
    const ordersToExpire = await query(`SELECT id, user_id, merchant_id, buyer_merchant_id, status, type, crypto_amount, fiat_amount, fiat_currency, escrow_tx_hash, accepted_at
     FROM orders
    ...
    if (acceptedExpired.length > 0) {
        ...
        const updateResult = await query(`UPDATE orders
       SET
         status = CASE
    ...
    if (MOCK_MODE) {
        for (const order of ordersToExpire) {
            if (order.escrow_tx_hash) {
                ...
                try {
                    await query(`UPDATE ${refundTable} SET balance = balance + $1 WHERE id = $2`, [amount, refundId]);
    ...
    for (const order of ordersToExpire) {
        ...
        try {
            ...
            await recordReputationEvent(order.user_id, 'user', eventType, reason, { orderId: order.id, previousStatus: order.status, amount: order.fiat_amount, currency: order.fiat_currency });
            // Record event for merchant
            await recordReputationEvent(order.merchant_id, 'merchant', eventType, reason, { orderId: order.id, previousStatus: order.status, amount: order.fiat_amount, currency: order.fiat_currency });
