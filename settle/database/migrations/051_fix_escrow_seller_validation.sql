-- Migration 051: Fix escrow_order_v1 — enforce seller-side escrow locking
--
-- BUG: escrow_order_v1 deducted balance from whoever called it (p_actor_id),
-- without verifying they are the correct seller. This allowed the buyer to lock
-- their own funds, which got credited back on release — zero net transfer.
--
-- FIX: Determine the correct seller from order.type, validate the caller IS the
-- seller, deduct from the seller, and record who was debited in the tracking columns
-- (escrow_debited_entity_type/id/amount/at) for deterministic refunds.
--
-- Seller logic:
--   buy  order → merchant_id  is the seller (merchant has USDC to sell)
--   sell order → user_id      is the seller (user has USDC to sell)
...
BEGIN
  ...
  -- 4. Determine the correct seller based on order type
  --    buy  = user buys USDC  → merchant_id is the seller (has USDC)
  --    sell = user sells USDC → user_id is the seller (has USDC)
  IF v_order.type = 'buy'::offer_type THEN
