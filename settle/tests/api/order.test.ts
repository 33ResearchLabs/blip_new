/**
 * API Tests — Order Action Endpoint + Enrichment Contract
 *
 * POST /api/orders/{id}/action — full lifecycle for BUY, SELL, M2M
 * GET  /api/orders/{id}        — BackendOrder contract compliance
 *
...
describe('Order Action API — POST /api/orders/{id}/action', () => {
  let fixtures: any;
  ...
  describe('User BUY: full happy path', () => {
    ...
    beforeAll(async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 500 });
      orderId = order.id;
    });
    ...
    it('CONFIRM_PAYMENT: seller confirms → completed', async () => {
      const result = await dispatchAction(orderId, {
  ...
  describe('User SELL: full happy path', () => {
    ...
    beforeAll(async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: buyOffer, type: 'sell', amount: 300 });
      orderId = order.id;
    });
    ...
    it('accepts → escrows → payment → completed', async () => {
      ...
      const escrowResult = await dispatchAction(orderId, {
        action: 'LOCK_ESCROW', actor_id: u1, actor_type: 'user',
      ...
      const payResult = await dispatchAction(orderId, {
        action: 'SEND_PAYMENT', actor_id: m1, actor_type: 'merchant',
      });
      ...
      const confirmResult = await dispatchAction(orderId, {
        action: 'CONFIRM_PAYMENT', actor_id: u1, actor_type: 'user',
      });
  ...
  describe('M2M BUY: merchant-to-merchant', () => {
    ...
    beforeAll(async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 1000 });
      orderId = order.id;
      await transitionOrder(orderId, 'accepted', 'merchant', m2);
    });

    it('both merchants have defined roles', async () => {
    ...
    it('transitions through full lifecycle', async () => {
      ...
      expect((await getSettleOrder(orderId, m1, 'merchant')).status).toMatch(/escrowed/);

      await transitionOrder(orderId, 'payment_sent', 'user', u1);
      expect((await getSettleOrder(orderId, m1, 'merchant')).status).toMatch(/payment_sent/);
    });
  });
  ...
  describe('M2M SELL: merchant-to-merchant', () => {
    ...
    beforeAll(async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: buyOffer, type: 'sell', amount: 750 });
      orderId = order.id;
      await transitionOrder(orderId, 'accepted', 'merchant', m2);
    });
  ...
  describe('CANCEL action', () => {
    it('cancels open order', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 100 });
      const result = await dispatchAction(order.id, {
        action: 'CANCEL', actor_id: u1, actor_type: 'user', reason: 'Test cancel from open',
      });
      expect(result.body.success).toBe(true);
    ...
    it('cancels accepted order', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 110 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);
      const result = await dispatchAction(order.id, {
        action: 'CANCEL', actor_id: m1, actor_type: 'merchant', reason: 'Test cancel from accepted',
      });
      expect(result.body.success).toBe(true);
      expect(result.body.newStatus).toBe('cancelled');
    });
    ...
    it('rejects cancel from payment_sent', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 130 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);
      await lockEscrowViaCore(order.id, 'merchant', m1);
      await transitionOrder(order.id, 'payment_sent', 'user', u1);
      const result = await dispatchAction(order.id, {
        action: 'CANCEL', actor_id: u1, actor_type: 'user',
      });
  ...
  describe('DISPUTE action', () => {
    it('disputes from escrowed', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 200 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);
      await lockEscrowViaCore(order.id, 'merchant', m1);
      const result = await dispatchAction(order.id, {
        action: 'DISPUTE', actor_id: u1, actor_type: 'user', reason: 'Test dispute',
      });
      expect(result.body.success).toBe(true);
    ...
    it('disputes from payment_sent', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 210 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);
      await lockEscrowViaCore(order.id, 'merchant', m1);
      await transitionOrder(order.id, 'payment_sent', 'user', u1);
      const result = await dispatchAction(order.id, {
        action: 'DISPUTE', actor_id: m1, actor_type: 'merchant', reason: 'Fiat not received',
      });
      expect(result.body.success).toBe(true);
      expect(result.body.newStatus).toBe('disputed');
    });
    ...
    it('rejects dispute from open', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 220 });
      const result = await dispatchAction(order.id, {
        action: 'DISPUTE', actor_id: u1, actor_type: 'user', reason: 'Too early',
      });
      expect(result.body.success).toBe(false);
      expect(result.body.code).toBe('INVALID_STATUS_FOR_ACTION');
    });
  ...
  describe('Response contract', () => {
    it('successful action returns all required fields', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 50 });
      const result = await dispatchAction(order.id, {
        action: 'CANCEL', actor_id: u1, actor_type: 'user', reason: 'Contract test',
      });
    ...
    it('failed action returns error and code', async () => {
      const result = await dispatchAction('00000000-0000-0000-0000-000000000000', {
        action: 'ACCEPT', actor_id: m1, actor_type: 'merchant',
      });
      expect(result.body.success).toBe(false);
    ...
    it('GET /api/orders/{id} returns full BackendOrder contract', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 75 });
  ...
  // ══════════════════════════════════════════════════════════════════════
  // Enricher ↔ Action Handler Consistency
  // ══════════════════════════════════════════════════════════════════════

  describe('Enricher ↔ Action handler consistency', () => {
    it('if primaryAction.type is set and enabled, dispatching it succeeds', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 300 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);
    ...
    it('if primaryAction.enabled is false, dispatching fails', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 310 });
      const enriched = await getSettleOrder(order.id, u1, 'user');
      expect(enriched.primaryAction.enabled).toBe(false);
  ...
  describe('Idempotency for financial actions', () => {
    it('duplicate LOCK_ESCROW: second attempt fails gracefully', async () => {
      const order = await createOrder({ userId: u1, merchantId: m1, offerId: sellOffer, type: 'buy', amount: 180 });
      await transitionOrder(order.id, 'accepted', 'merchant', m1);

      const r1 = await dispatchAction(order.id, {
        action: 'LOCK_ESCROW', actor_id: m1, actor_type: 'merchant',
        tx_hash: `idempotent_${Date.now()}`,
      });
      expect(r1.body.success).toBe(true);

      const r2 = await dispatchAction(order.id, {
        action: 'LOCK_ESCROW', actor_id: m1, actor_type: 'merchant',
        tx_hash: `idempotent_2_${Date.now()}`,
      });
