 ...
 */
export async function verifyReleaseInvariants(
  check: ReleaseInvariantCheck
): Promise<void> {
  const order = await queryOne<Order>('SELECT * FROM orders WHERE id = $1', [check.orderId]);

  if (!order) {
    throw new FinalizationInvariantError(
      'FINALIZATION_INVARIANT_BROKEN',
      check.orderId,
      { reason: 'order_not_found' },
      `Order ${check.orderId} not found after release commit`
    );
  }
 ...
 */
export async function verifyRefundInvariants(
  check: RefundInvariantCheck
): Promise<void> {
  const order = await queryOne<Order>('SELECT * FROM orders WHERE id = $1', [check.orderId]);
  ...
  if (!order) {
    throw new FinalizationInvariantError(
      ...
      check.orderId,
      { reason: 'order_not_found' },
      `Order ${check.orderId} not found after refund commit`
    );
  }
