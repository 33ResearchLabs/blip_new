...
function _redactParams(params) {
    if (!params)
        return undefined;
    return params.slice(0, 4).map((p) => {
        if (typeof p === 'string' && p.length > 64)
            return p.slice(0, 32) + '…';
        if (typeof p === 'string' && /token|password|secret|hash|key/i.test(p))
            return '[redacted]';
        return p;
    });
}
async function _trackDbError(sql, params, err, duration) {
    try {
        if (sql.toLowerCase().includes('error_logs'))
...
async function _trackSlowQuery(sql, params, duration, rows) {
    try {
        if (sql.toLowerCase().includes('error_logs'))
            return;
        const { safeLog } = await import('../errorTracking/logger');
 ...
 */
export async function transactionWithRetry(callback, opts = {}) {
