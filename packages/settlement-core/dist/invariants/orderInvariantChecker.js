...
export function checkInvariants(order, events, ledgerEntries) {
    const violations = [];
    ...
    for (const e of chronological) {
        if (hitTerminal && e.new_status && !TERMINAL_STATUSES.has(e.new_status)) {
            violations.push(`Event after terminal status: ${e.event_type} moved to ${e.new_status}`);
        }
...
export function checkPreCommit(order, intendedStatus, context) {
    // 1. No transition from terminal status
    if (TERMINAL_STATUSES.has(order.status)) {
        throw new PreCommitInvariantError('TERMINAL_STATUS', `Cannot transition from terminal status '${order.status}'`);
    }
    ...
    if (intendedStatus === 'completed' &&
        ...
        throw new PreCommitInvariantError('COMPLETION_WITHOUT_RELEASE', 'Cannot complete order with escrow but without release_tx_hash');
    }
    ...
    if (order.merchant_id &&
        ...
        throw new PreCommitInvariantError('SELF_REFERENCE', 'Order has merchant_id === buyer_merchant_id — blocked for non-accept/cancel transitions');
    }
    // 4. No API writes of transient statuses
    if (TRANSIENT_STATUSES.has(intendedStatus)) {
        throw new PreCommitInvariantError('TRANSIENT_STATUS', `Cannot write transient status '${intendedStatus}' via API`);
    }
}
//# sourceMappingURL=orderInvariantChecker.js.map