...
function normalizeSeverity(s) {
    if (s === 'INFO' || s === 'WARN' || s === 'ERROR' || s === 'CRITICAL')
        return s;
    return 'ERROR';
}
function normalizeSource(s) {
    if (s === 'frontend' || s === 'backend' || s === 'worker')
        return s;
    return 'backend';
}
...
export async function logServerError(payload) {
    if (!ENABLED)
        return;
    try {
        ...
        await query(`INSERT INTO error_logs (type, message, severity, order_id, user_id, merchant_id, source, metadata)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [type, message, severity, orderId, userId, merchantId, source, metadata]);
    }
    catch (err) {
        try {
...
export function safeLog(payload) {
    try {
