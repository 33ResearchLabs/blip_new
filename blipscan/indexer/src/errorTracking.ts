/**
 * Error tracking for the Blipscan indexer.
 *
 * Writes to the shared `error_logs` table (settle migration 088) in the
 * same Postgres database the indexer is already connected to. Pure
 * additive — no existing indexer behavior changes.
 *
 * Feature-gated via ENABLE_ERROR_TRACKING. When the flag is off every
 * call is a no-op.
 */
...
export function initErrorTracking(pool: Pool): void {
  _pool = pool;
}
  ...
  try {
    ...
    await _pool.query(
      `INSERT INTO error_logs (type, message, severity, order_id, user_id, merchant_id, source, metadata)
       VALUES ($1, $2, $3, $4, $5, $6, 'worker', $7)`,
      [type, message, severity, orderId, userId, merchantId, metadata]
    );
  } catch (err) {
    // Logging failures must not cascade — swallow and emit to stderr
    try {
      // eslint-disable-next-line no-console
      console.error('[blipscan-indexer errorTracking] insert failed', err);
    } catch { /* swallow */ }
  }
}
